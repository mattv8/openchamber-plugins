...
import type { GraphCommit as GitHistoryItem, GraphRef as GitHistoryRef } from '../domain/graph.js';
...
export type GitHistoryGraphRef = GitHistoryRef & { color?: string | undefined };
export type GitHistoryGraphItem = GitHistoryItem & {
  displayId?: string | undefined;
  references?: GitHistoryGraphRef[] | undefined;
};
...
function findLastIndex<T>(items: readonly T[], predicate: (item: T) => boolean): number {
  for (let index = items.length - 1; index >= 0; index--) {
    // SAFETY: index starts at items.length - 1 and decrements while non-negative.
...
function findLastNodeIndex(nodes: readonly GitHistoryGraphNode[], id: string): number {
  for (let index = nodes.length - 1; index >= 0; index--) {
    // SAFETY: index starts at nodes.length - 1 and decrements while non-negative.
...
function addIncomingOutgoingChangesHistoryItems(
  ...
  if (addIncomingChanges && upstream?.revision && upstream.revision !== mergeBase) {
    ...
    if (beforeHistoryItemIndex !== -1 && afterHistoryItemIndex !== -1) {
      // SAFETY: the checked find results are valid indexes in viewModels.
      const beforeHistoryItem = viewModels[beforeHistoryItemIndex]!;
      // SAFETY: the checked find result is a valid index in viewModels.
      const afterHistoryItem = viewModels[afterHistoryItemIndex]!;
      const incomingChangeMerged =
        beforeHistoryItem.historyItem.parentIds.length === 2 &&
        beforeHistoryItem.historyItem.parentIds.includes(mergeBase);

      if (!incomingChangeMerged) {
        const updatedBeforeHistoryItem: GitHistoryItemViewModel = {
          ...beforeHistoryItem,
          inputSwimlanes: beforeHistoryItem.inputSwimlanes.map((node) => (
            node.id === mergeBase && node.color === upstreamColor
              ? { ...node, id: SCMIncomingHistoryItemId }
              : node
          )),
          outputSwimlanes: beforeHistoryItem.outputSwimlanes.map((node) => (
            node.id === mergeBase && node.color === upstreamColor
              ? { ...node, id: SCMIncomingHistoryItemId }
              : node
          )),
        };
        viewModels[beforeHistoryItemIndex] = updatedBeforeHistoryItem;

        const inputSwimlanes = updatedBeforeHistoryItem.outputSwimlanes.map(cloneNode);
        const outputSwimlanes = afterHistoryItem.inputSwimlanes.map(cloneNode);
  ...
  if (addOutgoingChanges && current?.revision && current.revision !== mergeBase) {
    ...
    if (currentIndex !== -1) {
      ...
      const inputSwimlanes = currentViewModel.inputSwimlanes.map(cloneNode);
      ...
      // SAFETY: splice inserts before the current view model at currentIndex.
...
export function buildGitHistoryViewModels(
  ...
  const assignColor = (identity: string, unavailableColors: readonly string[] = []): string => {
    ...
    // SAFETY: preferredIndex is modulo colorRegistry.length.
    ...
    if (unavailable.size < colorRegistry.length) {
      for (let index = 0; index < colorRegistry.length; index++) {
        // SAFETY: rotate returns a value in [0, colorRegistry.length).
  ...
  for (const historyItem of items) {
        ...
        if (node.id === graphHistoryItem.id) {
          if (!firstParentAdded) {
            outputSwimlanes.push({
              // SAFETY: enclosing branch establishes at least one parent.
    ...
    for (let index = firstParentAdded ? 1 : 0; index < graphHistoryItem.parentIds.length; index++) {
      // SAFETY: index is bounded by graphHistoryItem.parentIds.length.
      const parentId = graphHistoryItem.parentIds[index]!;
      let color = index === 0 ? labelColor : undefined;

      if (index > 0) {
        const parentItem = items.find((candidate) => candidate.id === parentId);
        color = parentItem ? getOrAssignLabelColor(parentItem, outputSwimlanes.map((node) => node.color)) : undefined;
      }

      if (!color) {
        color = assignColor(parentId, outputSwimlanes.map((node) => node.color));
      }
...
export function getHistoryItemSecondaryParentColumns(viewModel: GitHistoryItemViewModel): number[] {
  ...
  for (let index = 1; index < viewModel.historyItem.parentIds.length; index++) {
    // SAFETY: index is bounded by viewModel.historyItem.parentIds.length.

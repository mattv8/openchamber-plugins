...
export const GitGraphSegment: React.FC<GitGraphSegmentProps> = ({ viewModel, totalColumns }) => {
  ...
  for (let index = 0; index < inputSwimlanes.length; index++) {
    // SAFETY: index is bounded by inputSwimlanes.length.
    const inputSwimlane = inputSwimlanes[index]!;
    const color = inputSwimlane.color;

    if (inputSwimlane.id === historyItem.id) {
    ...
    if (
      ...
      // SAFETY: the preceding condition bounds outputSwimlaneIndex.
      && inputSwimlane.id === outputSwimlanes[outputSwimlaneIndex]!.id
    ) {
  ...
  if (inputIndex !== -1) {
    paths.push(
      <GraphPath
        ...
        // SAFETY: inputIndex comes from findIndex over inputSwimlanes.

...
import {
  ...
  type GitHistoryItem,
  type GitHistoryRef,
} from '../../src/panel/original/gitGraph.js';

function fixtureItem<T>(items: readonly T[], index: number): T {
...
describe('buildGitHistoryViewModels', () => {
  ...
  test('builds a linear history', () => {
    ...
    expect(fixtureItem(viewModels, 0).inputSwimlanes).toHaveLength(0);
    expect(fixtureItem(viewModels, 0).outputSwimlanes).toEqual([{ id: 'b', color: 'var(--git-graph-3)' }]);
    expect(fixtureItem(viewModels, 1).inputSwimlanes).toEqual([{ id: 'b', color: 'var(--git-graph-3)' }]);
    expect(fixtureItem(viewModels, 4).outputSwimlanes).toHaveLength(0);
  });
  ...
  test('assigns an unlabelled first parent from its FNV-1a commit identity instead of sequence position', () => {
    ...
    expect(fixtureItem(viewModels, 0).outputSwimlanes).toEqual([{ id: 'b', color: 'var(--git-graph-3)' }]);
    expect(fixtureItem(viewModels, 1).nodeColor).toBe('var(--git-graph-3)');
  });
  ...
  test('keeps divergence and merge swimlanes stable', () => {
    ...
    expect(fixtureItem(viewModels, 0).outputSwimlanes).toEqual([
      { id: 'b', color: 'var(--git-graph-3)' },
      { id: 'c', color: 'var(--git-graph-4)' },
    ]);
    expect(fixtureItem(viewModels, 1).inputSwimlanes).toEqual([
      { id: 'b', color: 'var(--git-graph-3)' },
      { id: 'c', color: 'var(--git-graph-4)' },
    ]);
    expect(fixtureItem(viewModels, 4).outputSwimlanes).toEqual([
  ...
  test('handles branches created from merge commits', () => {
    ...
    expect(fixtureItem(viewModels, 2).inputSwimlanes).toEqual([
      { id: 'b', color: 'var(--git-graph-3)' },
      { id: 'b', color: 'var(--git-graph-4)' },
    ]);
    expect(fixtureItem(viewModels, 2).outputSwimlanes).toEqual([
      { id: 'd', color: 'var(--git-graph-3)' },
  ...
  test('keeps ref colors deterministic for non-tag refs while tags stay neutral', () => {
    ...
    expect(fixtureItem(viewModels, 0).historyItem.references?.map((ref) => ref.id)).toEqual([
      topic.id,
      release.id,
      tag.id,
    ]);
    expect(fixtureItem(viewModels, 0).historyItem.references?.map((ref: GitHistoryGraphRef) => ref.color)).toEqual(
      fixtureItem(rebuilt, 0).historyItem.references?.map((ref: GitHistoryGraphRef) => ref.color),
    );
    expect(/^var\(--git-graph-[1-5]\)$/.test(fixtureItem(viewModels, 0).historyItem.references?.[0]?.color ?? '')).toBe(true);
    expect(/^var\(--git-graph-[1-5]\)$/.test(fixtureItem(viewModels, 0).historyItem.references?.[1]?.color ?? '')).toBe(true);
    expect(fixtureItem(viewModels, 0).historyItem.references?.map((ref: GitHistoryGraphRef) => ref.color)).toEqual([
      fixtureItem(viewModels, 0).historyItem.references?.[0]?.color,
      fixtureItem(viewModels, 0).historyItem.references?.[1]?.color,
      undefined,
    ]);
  });
  ...
  test('keeps current and upstream ref identities on distinct graph slots', () => {
    ...
    expect(/^var\(--git-graph-[1-5]\)$/.test(fixtureItem(viewModels, 0).historyItem.references?.[0]?.color ?? '')).toBe(true);
    expect(/^var\(--git-graph-[1-5]\)$/.test(fixtureItem(viewModels, 2).historyItem.references?.[0]?.color ?? '')).toBe(true);
    expect(/^var\(--git-graph-[1-5]\)$/.test(fixtureItem(viewModels, 5).historyItem.references?.[0]?.color ?? '')).toBe(true);
    expect(fixtureItem(viewModels, 0).historyItem.references?.[0]?.color).not.toBe(fixtureItem(viewModels, 2).historyItem.references?.[0]?.color);
    expect(fixtureItem(viewModels, 0).historyItem.references?.map((ref) => ref.id)).toEqual([current.id]);
    expect(fixtureItem(viewModels, 2).historyItem.references?.map((ref) => ref.id)).toEqual([upstream.id]);
  });
  ...
  test('adds incoming and outgoing synthetic nodes around the merge base', () => {
    ...
    if (outgoingRefColor === undefined || incomingRefColor === undefined) throw new Error('Missing fixture ref color');
    ...
    expect(fixtureItem(viewModels, 5).inputSwimlanes).toEqual([
      { id: 'scm-graph-incoming-changes', color: incomingRefColor },
      { id: 'e', color: outgoingRefColor },
    ]);
    expect(fixtureItem(viewModels, 2).nodeColor).toBe(outgoingRefColor);
    expect(fixtureItem(viewModels, 5).nodeColor).toBe(incomingRefColor);
  });
  ...
  test('preserves unresolved parents on partial pages', () => {
    ...
    expect(fixtureItem(viewModels, 1).outputSwimlanes).toEqual([
      { id: 'b', color: 'var(--git-graph-3)' },
      { id: 'd', color: 'var(--git-graph-4)' },
    ]);
  });
  ...
  test('keeps existing swimlanes stable when additional history is appended', () => {
    ...
    expect(fixtureItem(appended, 0).outputSwimlanes).toEqual(fixtureItem(firstPage, 0).outputSwimlanes);
    expect(fixtureItem(appended, 1).inputSwimlanes).toEqual(fixtureItem(firstPage, 1).inputSwimlanes);
    expect(fixtureItem(appended, 1).outputSwimlanes).toEqual(fixtureItem(firstPage, 1).outputSwimlanes);
  ...
  test('assigns stable theme color slots from ref and commit identities across rebuilds', () => {
    ...
    expect(/^var\(--git-graph-[1-5]\)$/.test(fixtureItem(first, 0).nodeColor)).toBe(true);
    expect(/^var\(--git-graph-[1-5]\)$/.test(fixtureItem(first, 2).nodeColor)).toBe(true);
    expect(fixtureItem(first, 0).nodeColor).not.toBe(fixtureItem(first, 2).nodeColor);
  });
  ...
  test('avoids active-lane slot collisions while palette capacity remains', () => {
    ...
    expect(fixtureItem(viewModels, 0).outputSwimlanes[0]?.color).toBe('var(--git-graph-4)');
    expect(fixtureItem(viewModels, 0).outputSwimlanes[1]?.color).not.toBe('var(--git-graph-4)');
    expect(fixtureItem(viewModels, 0).outputSwimlanes[0]?.color).not.toBe(fixtureItem(viewModels, 0).outputSwimlanes[1]?.color);
  });
  ...
  test('assigns a deterministic graph slot to root commits', () => {
    ...
    expect(fixtureItem(first, 0).nodeColor).toBe('var(--git-graph-4)');
    expect(fixtureItem(first, 0).nodeColor).toBe(fixtureItem(rebuilt, 0).nodeColor);
  });
  ...
  test('probes labelled refs to different active-lane slots on identity collision', () => {
    ...
    const refAColor = fixtureItem(viewModels, 0).historyItem.references?.[0]?.color;
    const refFColor = fixtureItem(viewModels, 2).historyItem.references?.[0]?.color;
    ...
    expect(fixtureItem(viewModels, 2).outputSwimlanes).toHaveLength(2);
    expect(fixtureItem(viewModels, 2).outputSwimlanes[0]?.color).not.toBe(fixtureItem(viewModels, 2).outputSwimlanes[1]?.color);
    expect(fixtureItem(viewModels, 2).outputSwimlanes.find((lane) => lane.id === 'g')?.color).toBe(refFColor);
  });
});

...
import { buildGitRefBadgePresentation, getGitRefBadgeIcon } from '../../src/panel/original/gitRefBadges.js';
import type { GitHistoryGraphRef } from '../../src/panel/original/gitGraph.js';

const ref = (id: string, name: string, kind: GitHistoryGraphRef['kind'], color?: string): GitHistoryGraphRef => {
  const reference: GitHistoryGraphRef = {
    id,
    name,
    kind,
    revision: 'commit',
  ...
  if (color !== undefined) reference.color = color;
  return reference;
};
...
describe('buildGitRefBadgePresentation', () => {
  ...
  test('keeps ranked current and upstream references in order while compacting the upstream name', () => {
    const references: [GitHistoryGraphRef, GitHistoryGraphRef, GitHistoryGraphRef] = [
  ...
  test('prioritizes non-tag refs, showing first tag as primary only when all refs are tags', () => {
    const tagOnly: [GitHistoryGraphRef] = [ref('refs/tags/v1', 'v1', 'tag')];
  ...
  test('handles detached HEAD (kind=head without matching branch)', () => {
    const references: [GitHistoryGraphRef, GitHistoryGraphRef] = [
  ...
  test('preserves upstream before unrelated local refs in ranked order', () => {
    const references: [GitHistoryGraphRef, GitHistoryGraphRef, GitHistoryGraphRef, GitHistoryGraphRef] = [
      ref('refs/heads/beta', 'beta', 'head', 'var(--git-graph-1)'),
      ref('refs/remotes/origin/beta', 'origin/beta', 'remote', 'var(--git-graph-2)'),
